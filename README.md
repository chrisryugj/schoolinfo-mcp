...
**여러 학교를 한 표로 비교**하고, hwp 첨부에 묻힌 **수행평가 계획**을 표로 풀어 봅니다. 학생수·동아리·방과후 같은 공시와 **급식(알레르기 회피 필터)·학사일정·시험 D-day·이번주 브리핑까지** 한 번에 받게 해줍니다.
...
코드는 MIT([LICENSE](./LICENSE)). **데이터에는 MIT가 적용되지 않습니다.**

| 데이터 | 출처 | 조건 |
|--------|------|------|
| 학교 공시정보 (정형 35종) | [학교알리미](https://www.schoolinfo.go.kr) OpenAPI | 공공누리 제1유형(출처표시) |
| 학사일정 | NEIS 교육정보 개방포털 | 각 포털 이용약관 |
| 전공 연계 권장과목 (`src/admission.json`) | 「2028학년도 권역별 대학별 권장과목(반영과목)」(한국대학교육협의회) 외 — 파일 내 `sources` 참조 | 아래 참고 |
| hwp 변환 엔진 | [kordoc](https://github.com/chrisryugj/kordoc) | MIT |

`src/admission.json` 은 대학별 권장과목 안내 자료의 기재 내용을 원문에 가깝게 옮긴
것입니다. 이 저장소의 MIT 라이선스는 해당 데이터에 적용되지 않으며, 권리는 원 발간
기관에 있습니다. 재사용하려면 원 자료의 이용 조건을 직접 확인하세요. 입시 판단에는
반드시 각 대학의 최신 공식 발표를 확인해야 합니다.

전체 고지는 [NOTICE](./NOTICE)를 참조하세요.

---

<sub>이 도구는 개인이 만든 비공식 프로젝트이며, 학교알리미·NEIS·한국대학교육협의회 등
어떤 기관과도 제휴·후원 관계가 없습니다. 서비스명 「우리 학교 알리미」는 공적 서비스
「학교알리미」와 무관합니다.</sub>
</content>
</invoke>
